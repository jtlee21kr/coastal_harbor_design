...
for idx, row in enumerate(valid_tiers):
    ...
    if is_first:
        ...
        # 💡 [수정] 상치부 로컬 X좌표에 x_front(예: -0.5) 오프셋을 반영해야 1.75가 나옵니다!
        # 기존: x_local_rect = w_rect / 2.0
        x_local_rect = x_front + (w_rect / 2.0)  # -0.5 + 4.5/2 = 1.75
        x_global_rect = x_local_rect  # 상치부 자체가 첫 단이므로 글로벌=로컬

        x_local_tri = (x_front + w_rect + w_tri / 3.0) if b > top_b else (x_front + w_tri * (2.0 / 3.0))
        x_global_tri = x_local_tri  # ★ 공백 제거 완료
