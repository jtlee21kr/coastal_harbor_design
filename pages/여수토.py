...
def load_dredging_data(file_path):
    try:
        ...
        interp_func = RegularGridInterpolator((distances, n_values), q_matrix, method='linear', bounds_error=False,
                                              fill_value=None)
...
class SpillwayDesign:
    ...
    def calculate_discharge(self, area_ha, t_min, runoff_coeff, dredger_count, qs_val, mud_ratio, ksce_data,
                            poly_coeffs, gen_coeffs):
        ...
        ln_I = a1 + b1 * (ln_t) + c1 * (ln_t ** 2) + d1 * (ln_t ** 3) + e1 * (ln_t ** 4) + f1 * (ln_t ** 5) + g1 * (
                    ln_t ** 6)
...
def main():
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            st.latex(
                rf"Q_s = q \times \frac{{b_0}}{{746}} \times E = {q_read:,.2f} \times \frac{{{b0_kw_res:,.2f}}}{{746}} \times {eff_total:.2f} = {qs_res:,.2f} \, m^3/hr")
    ...
    with st.container(border=True):
        ...
        st.latex(
            rf"Q_1 = \frac{{{qs_res:,.2f} \times {d_count} \times (1 + {calc_water_ratio:.2f})}}{{3600}} = {calc_dredge_q:,.3f} \, m^3/sec")
    ...
    with st.container(border=True):
        ...
        st.latex(
            rf"t_c = 1.44 \times \left( \frac{{{kerby_L} \times {kerby_n}}}{{\sqrt{{{kerby_S}}}}} \right)^{{0.467}} = {t_min_raw:.2f} \, \text{{min}}")
    ...
    with c2_5:
        ...
        st.markdown("**[🔗 전국 하천유역 홍수량 및 확률강우량 정보 (WAMIS)](https://map.wamis.go.kr/)**",
                    help="클릭 시 새 창에서 열립니다. 해당 사이트에서 지역별 계수를 확인하여 아래에 직접 입력해 주세요.")
    ...
    with st.container(border=True):
        ...
        st.write(
            f"📌 **입력된 계수:** `a={a1:.6f}`, `b={b1:.6f}`, `c={c1:.6f}`, `d={d1:.6f}`, `e={e1:.6f}`, `f={f1:.6f}`, `g={g1:.6f}`")
    ...
    with st.container(border=True):
        ...
        st.latex(
            rf"Q = \frac{{1}}{{3.6}} \times {run_c} \times {rain_i_tomok:.2f} \times {area_ha / 100:.4f} = {q_tomok:.3f} \, m^3/sec")

        st.write(f"**② 전대수다항식 유출량 산정:**")
        st.latex(
            rf"Q = \frac{{1}}{{3.6}} \times {run_c} \times {rain_i_poly:.2f} \times {area_ha / 100:.4f} = {q_poly:.3f} \, m^3/sec")

        st.write(f"**③ GENERAL식 유출량 산정:**")
        st.latex(
            rf"Q = \frac{{1}}{{3.6}} \times {run_c} \times {rain_i_gen:.2f} \times {area_ha / 100:.4f} = {q_gen:.3f} \, m^3/sec")
    ...
    with st.container(border=True):
        st.write("**[손실계수 산출]**")
        st.latex(
            rf"f_2 = \frac{{124.5 \times n^2}}{{D^{{4/3}}}} \times L = \frac{{124.5 \times {pipe_n:.3f}^2}}{{{pipe_D:.2f}^{{4/3}}}} \times {pipe_L:.2f} = {f2_coeff:.3f}")
        st.latex(
            rf"\Sigma f = f_1 + f_2 + f_3 = {f1_coeff:.2f} + {f2_coeff:.3f} + {f3_coeff:.2f} = {total_loss_coeff:.3f}")

        st.divider()
        st.write("**[고조시 최대 유속 및 유량 산출]**")
        st.latex(r"V = \sqrt{\frac{2g \Delta H}{\Sigma f}}")
        st.latex(
            rf"V = \sqrt{{\frac{{2 \times {g_val} \times {delta_H:.2f}}}{{{total_loss_coeff:.3f}}}}} = {pipe_V_sub:.3f} \, m/s")
    ...
    with st.container(border=True):
        ...
        * $g$ : 중력가속도 ($9.8 \, m/sec^2$)
        ...
        st.latex(
            rf"{q_per_basin:.4f} = \left( 0.405 + \frac{{0.003}}{{{weir_H:.2f}}} \right) \times B \times \sqrt{{2 \times 9.8}} \times {weir_H:.2f}^{{1.5}}")
        st.latex(
            rf"{q_per_basin:.4f} = {bazin_C:.4f} \times B \times {math.sqrt(2 * g_val):.4f} \times {weir_H ** 1.5:.4f}")
        st.latex(
            rf"B = \frac{{{q_per_basin:.4f}}}{{{bazin_C * math.sqrt(2 * g_val) * (weir_H ** 1.5):.4f}}} = {req_weir_B:.3f} \, m")

        st.info(f"👉 **집수정당 소요 월류폭 계산값** = {req_weir_B:.3f} m 이상")

        st.divider()
        st.markdown("#### ⚙️ 월류폭 최종 결정")
        final_weir_B = st.number_input("설계에 반영할 최종 집수정당 월류폭 (m)",
                                       min_value=math.ceil(req_weir_B * 10) / 10.0 if req_weir_B > 0 else 1.0,
                                       value=max(2.0, math.ceil(req_weir_B * 10) / 10.0), step=0.1)
    ...
    with st.container(border=True):
        ...
        st.latex(
            r"d = \frac{V^2}{2g \cdot y^2 \left(\frac{\gamma_r}{\gamma_w} - 1\right)}, \quad W = \frac{\pi d^3}{6} \gamma_r")
        st.markdown("""
        * $V$ : 설계 유속 ($m/s$)
        * $g$ : 중력가속도 ($9.8 \, m/s^2$)
        ...
        * $\gamma_r$, $\gamma_w$ : 피복석 및 해수 단위중량 ($kN/m^3$)
        ...
        st.latex(
            rf"d = \frac{{{target_V:.2f}^2}}{{2 \times 9.8 \times {isbash_y:.2f}^2 \times ({sr:.3f} - 1)}} = \frac{{{target_V ** 2:.4f}}}{{{2 * 9.8 * isbash_y ** 2 * (sr - 1):.4f}}} = {stone_d:.3f} \, m")
    ...
    with st.container(border=True):
        ...
        st.latex(
            rf"P_{{soil}} = K_o \times H_{{soil}} \times \gamma_{{sub}} = {ko} \times {h_soil} \times {gamma_sub_soil_b} = {p_soil:.3f} \, kN/m^2")
        st.latex(
            rf"P_{{water}} = K_o \times H_{{water}} \times \gamma_w = {ko} \times {h_water} \times {gamma_water_b} = {p_water:.3f} \, kN/m^2")
        ...
        st.latex(
            rf"W_{{max}} = P_{{total}} \times \text{{토류판 높이}} = {p_total:.3f} \times {wood_h} = {w_max:.3f} \, kN/m")
        st.latex(
            rf"M_{{max}} = \frac{{W_{{max}} \times l^2}}{{8}} = \frac{{{w_max:.3f} \times {wood_l}^2}}{{8}} = \mathbf{{{m_max:.3f} \, kN\cdot m}}")
        ...
        st.latex(
            rf"Z \, \text{{(단면계수)}} = \frac{{b \times t^2}}{{6}} = \frac{{{b_mm:.0f} \times {t_mm:.0f}^2}}{{6}} = {Z_mm3:.1f} \, mm^3")
        st.latex(
            rf"f \, \text{{(발생응력)}} = \frac{{M_{{max}}}}{{Z}} = \frac{{{m_max:.3f} \times 10^6}}{{{Z_mm3:.1f}}} = {stress_mpa:.3f} \, MPa")
    ...
    st.info(
        "💡 **수식 안 깨지게 활용하는 법:** 다운로드한 HTML 파일을 **웹 브라우저(크롬, 엣지 등)**로 엽니다. 브라우저에서 깔끔하게 렌더링된 수식을 확인한 뒤, 브라우저의 **'인쇄(Ctrl+P)' 기능으로 PDF로 저장**하거나 **전체 복사하여 한글(HWP)/워드에 붙여넣기** 하시면 수식이 완벽하게 복사됩니다.")
