...
if scour_status == "필요":
    ...
    st.latex(
        rf"U_z = \frac{{\pi \times {H_input:.2f}}}{{{T_input:.2f}}} \times \frac{{\cosh[2\pi({z_depth:.2f} + {h_bed:.2f})/{L_init:.2f}]}}{{\sinh(2\pi \times {h_bed:.2f} / {L_init:.2f})}} = {u_z:.4f} \, m/s")
    ...
    st.latex(
        rf"W = \frac{{\pi \times {gamma_r:.2f} \times ({u_z:.4f})^6}}{{48 \times ({g_val})^3 \times ({isbash_y})^6 \times ({S_r:.4f} - 1)^3 \times (\cos{theta_angle}^\circ - \sin{theta_angle}^\circ)^3}} = {W_wave_kN:.4f} \, kN")
    st.latex(
        rf"d = \left( \frac{{6W}}{{\pi \gamma_r}} \right)^{{1/3}} = \left( \frac{{6 \times {W_wave_kN:.4f}}}{{\pi \times {gamma_r:.2f}}} \right)^{{1/3}} = {d_wave:.3f} \, m")
    ...
    st.latex(
        rf"W = \frac{{\pi \times {gamma_r:.2f} \times ({v_tidal:.2f})^6}}{{48 \times ({g_val})^3 \times ({isbash_y})^6 \times ({S_r:.4f} - 1)^3 \times (\cos{theta_angle}^\circ - \sin{theta_angle}^\circ)^3}} = {W_current_kN:.4f} \, kN")
    st.latex(
        rf"d = \left( \frac{{6W}}{{\pi \gamma_r}} \right)^{{1/3}} = \left( \frac{{6 \times {W_current_kN:.4f}}}{{\pi \times {gamma_r:.2f}}} \right)^{{1/3}} = {d_current:.3f} \, m")
    ...
    if structure_type == "직립제 (Vertical)":
        if location_type == "제두부 (Head)":
            ...
            st.latex(
                rf"KC = \frac{{u_{{bottom}} T_s}}{{B}} = \frac{{{u_bottom:.3f} \times {T_input:.2f}}}{{{B_width}}} = {KC:.3f}")
            ...
            if "Xie" in wave_condition:
                ...
                st.latex(
                    r"(U_{rms})_m \text{ 적용 판별 } (k_p h) = " + f"{kph:.4f} \quad (0.05 < k_p h < 3.0) \rightarrow \mathbf{{{validity_str}}}")
                ...
                st.latex(
                    r"H_{mo} = \frac{H_s}{H_s / H_{mo}} = \frac{" + f"{H_input:.2f}" + r"}{" + f"{Hs_ratio:.2f}" + r"} = " + f"{Hmo:.2f} \, m")
                    ...
                    for series in unique_series:
                        ...
                        if len(cols) >= 2:
                            ...
                            if "maximum" in s_name_lower:
                                ...
                                x_tgt = 0.002
                                try:
                                    y_ptr = float(p_curve(x_tgt)) if p_curve else ey_u[len(ey_u) // 3]
                                except:
                                    y_ptr = ey_u[len(ey_u) // 3]
                            ...
                            # Average 렌더링
                                ...
                                x_tgt = 0.002
                                try:
                                    y_ptr = float(p_curve(x_tgt)) if p_curve else ey_u[1]
                                except:
                                    y_ptr = ey_u[1]
                ...
                st.latex(
                    r"(U_{rms})_m = \frac{\sqrt{2}}{4\pi \cosh(k_p h)} \times \left[ 0.54 \cosh\left(\frac{1.5 - k_p h}{2.8}\right) \right] \times g k_p T_p H_{mo}")
        ...
        st.latex(
            rf"\frac{{S_m}}{{H_s}} = 0.01 \times {Cu_input:.2f} \times \left( \frac{{{Tp:.2f} \times \sqrt{{{g_val} \times {H_input:.2f}}}}}{{{h_bed:.2f}}} \right)^{{1.5}} = {Sm_ratio:.4f}")
