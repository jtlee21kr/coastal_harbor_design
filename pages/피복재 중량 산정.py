...
if run_button:
    ...
    # 1. 쇄파대 검토 실행
    L0, X_h0, Hb_ratio, Hb, X_hb, alpha, beta, db_max, db_min, is_brk_spm, status_spm = calc.check_surf_zone_spm(
        H0_prime, Tz, m_slope, depth)
    ...
    rock_v_weight, r_type, _, r_Lom, r_sm, r_xim, r_ximc, r_Ns, r_Dn50 = calc.calc_vandermeer_rock(gamma_rock, Hs, Tz,
                                                                                                   cot_alpha, P_rock,
                                                                                                   S_rock, N_waves)
    ...
    st.markdown(
        rf"- 유의파고 $H_{{1/3}} = {Hs} \text{{ m}}$, 유의주기 $T_{{1/3}} = {Tz} \text{{ s}}$, 수심 $h = {depth} \text{{ m}}$")
    ...
    st.markdown(
        rf"*(해저경사 m={m_slope} 등 도표에 직접 곡선이 표시되지 않은 경사는, 앵커 곡선 데이터를 바탕으로 수학적 2D 정밀 보간법을 사용하여 오차 없이 독취 및 산출하였습니다.)*")
    ...
    st.markdown(
        rf"👉 **판정:** 수심 $h ({depth} \text{{ m}}) > d_{{b,max}} ({db_max:.3f} \text{{ m}})$ 여부 비교결과 **{status_spm}** 임.")
    ...
    st.markdown(
        rf"- 최대치 출현 수심 $(h_{{1/3}})_{{peak}} = {peak_ratio:.2f} \times {H0_prime:.3f} = {h_peak:.3f} \text{{ m}}$")
    st.markdown(
        rf"👉 **판정:** 수심 $h ({depth} \text{{ m}}) > (h_{{1/3}})_{{peak}} ({h_peak:.3f} \text{{ m}})$ 여부 비교결과 **{status_harbor}** 임.")
    ...
    st.markdown(f"- **안정계수($K_D$)**: {Kd_rock:.1f} (판정결과 자동적용), **사면경사({html_cot} $\\alpha$)**: {cot_alpha}",
                unsafe_allow_html=True)
    st.latex(
        r"W = \frac{\gamma_r H^3}{K_D \cdot \mathrm{C}\kern0.1ex\mathrm{o}\kern0.1ex\mathrm{t}\,\alpha \cdot (S_r - 1)^3}")
    st.latex(
        rf"W = \frac{{{gamma_rock:.2f} \times {H_design:.2f}^3}}{{{Kd_rock:.1f} \times {cot_alpha} \times ({r_Sr:.3f} - 1)^3}} = {rock_h_weight:,.1f} \text{{ kN}}")
    ...
    st.markdown(
        f"- **쇄파유사도($\\xi_m$)**: {html_tan} $\\alpha / \\sqrt{{s_m}} = (1/{cot_alpha}) / \\sqrt{{{r_sm:.4f}}} = {r_xim:.3f}$",
        unsafe_allow_html=True)
    st.markdown(
        f"- **임계 쇄파유사도($\\xi_{{mc}}$)**: $(6.2 \\times P^{{0.31}} \\sqrt{{\\mathrm{{t}}\\kern0.1ex\\mathrm{{a}}\\kern0.1ex\\mathrm{{n}}\\,\\alpha}})^{{1/(P+0.5)}} = {r_ximc:.3f}$ (투과계수 P={P_rock})",
        unsafe_allow_html=True)

    st.markdown(
        rf"- **파랑조건 판정**: $\xi_m({r_xim:.3f})$ {'<' if r_xim < r_ximc else '>'} $\xi_{{mc}}({r_ximc:.3f})$ 이므로 **{r_type}** 공식 적용")
    st.markdown(rf"- **안정계수($N_s$)**: {r_Ns:.3f} (허용손상도 S={S_rock}, 내습파랑수 N={N_waves})")
    st.markdown(
        rf"- **공칭직경($D_{{n50}}$)**: $H_s / ((S_r - 1) N_s) = {Hs} / (({r_Sr:.3f} - 1) \times {r_Ns:.3f}) = {r_Dn50:.3f} \text{{ m}}$")
    ...
    st.markdown(f"- **안정계수($K_D$)**: {Kd_ttp:.1f} (판정결과 자동적용), **사면경사({html_cot} $\\alpha$)**: {cot_alpha}",
                unsafe_allow_html=True)
    st.latex(
        r"W = \frac{\gamma_r H^3}{K_D \cdot \mathrm{C}\kern0.1ex\mathrm{o}\kern0.1ex\mathrm{t}\,\alpha \cdot (S_r - 1)^3}")
    st.latex(
        rf"W = \frac{{{gamma_ttp:.2f} \times {H_design:.2f}^3}}{{{Kd_ttp:.1f} \times {cot_alpha} \times ({t_Sr:.3f} - 1)^3}} = {ttp_h_weight:,.1f} \text{{ kN}}")
    ...
    st.markdown(
        rf"- **심해파장($L_{{om}}$)**: $g T_m^2 / 2\pi = 9.81 \times {t_Tm:.2f}^2 / 2\pi = {t_Lom:.2f} \text{{ m}}$")
    st.markdown(rf"- **파형경사($s_m$)**: $H_s / L_{{om}} = {Hs} / {t_Lom:.2f} = {t_sm:.4f}$")

    st.markdown(
        rf"- **안정계수($N_s$)**: $(3.75 \sqrt{{N_{{od}}}} / N^{{0.25}} + 0.85) \times s_m^{{-0.2}} = {t_Ns:.3f}$ (상대피해율 Nod={Nod_ttp})")
    st.markdown(
        rf"- **공칭직경($D_n$)**: $H_s / ((S_r - 1) N_s) = {Hs} / (({t_Sr:.3f} - 1) \times {t_Ns:.3f}) = {t_Dn:.3f} \text{{ m}}$")
    ...
    st.markdown(f"- **안정계수($K_D$)**: {Kd_other:.1f} (사용자 입력), **사면경사({html_cot} $\\alpha$)**: {cot_alpha}",
                unsafe_allow_html=True)
    st.latex(
        r"W = \frac{\gamma_r H^3}{K_D \cdot \mathrm{C}\kern0.1ex\mathrm{o}\kern0.1ex\mathrm{t}\,\alpha \cdot (S_r - 1)^3}")
    st.latex(
        rf"W = \frac{{{gamma_other:.2f} \times {H_design:.2f}^3}}{{{Kd_other:.1f} \times {cot_alpha} \times ({o_Sr:.3f} - 1)^3}} = {other_h_weight:,.1f} \text{{ kN}}")
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        df_summary = pd.DataFrame({
            ...
            "결과값": [Hs, depth, round(H0_prime, 3), status_spm, status_harbor, round(rock_final_kN, 1),
                    round(ttp_final_kN, 1), round(other_final_kN, 1)]
        })
    ...
    st.download_button("📥 전체 결과(요약+상세) 엑셀(.xlsx) 다운로드", data=output.getvalue(), file_name="coastal_armor_detailed.xlsx",
